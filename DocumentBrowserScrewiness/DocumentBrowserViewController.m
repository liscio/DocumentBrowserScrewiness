...
- (void)documentBrowser:(UIDocumentBrowserViewController *)controller didRequestDocumentCreationWithHandler:(void (^)(NSURL * _Nullable, UIDocumentBrowserImportMode))importHandler {
    ...
    NSURL *newDocumentURL = [NSURL fileURLWithPath:[[docPath stringByAppendingPathComponent:@"test"] stringByAppendingPathExtension:@"foobar"]];
    
    Document *d = [[Document alloc] initWithFileURL:newDocumentURL];
    [d saveToURL:newDocumentURL forSaveOperation:UIDocumentSaveForCreating completionHandler:^(BOOL success) {
